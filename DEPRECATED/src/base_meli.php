...
abstract class BaseMeli
{
     ...
     */
    public static $CURL_OPTS = [CURLOPT_USERAGENT => 'MeliPHP-sdk-0.0.4', CURLOPT_CONNECTTIMEOUT => 10, CURLOPT_RETURNTRANSFER => true, CURLOPT_TIMEOUT => 60];
     ...
     * the current URL.
     */
    protected static $DROP_QUERY_PARAMS = ['code', 'meli-logout', 'meli-refresh'];
    ...
    protected function getAccessTokenFromCode($code, $redirect_uri = null)
    {
        ...
        $result = $this->execute('POST', false, '/oauth/token', ['grant_type' => 'authorization_code', 'code' => $code, 'client_id' => $this->getAppId(), 'client_secret' => $this->getAppSecret(), 'redirect_uri' => $redirect_uri]);
        $json = $result['json'];
        if (!isset($json) || isset($json['error'])) {
            throw new AuthorizationException($result);
        }

        return ['value'                 => $json['access_token'],
    ...
    public function getAccessTokenFromRefreshToken()
    {
        ...
        $result = $this->execute('POST', false, '/oauth/token', [
            'grant_type'    => 'refresh_token',
            'client_id'     => $this->getAppId(),
            'client_secret' => $this->getAppSecret(),
            'refresh_token' => $accessToken['refresh_token'], ]);

        return ['value' => $result['json']['access_token'], 'expires' => time() + $result['json']['expires_in'], 'scope' => $result['json']['scope'], 'refresh_token' => $result['json']['refresh_token']];
    }
    ...
    public function getLoginUrl($params = [])
    {
        ...
        return $this->getUrl('auth', '/authorization', array_merge(['client_id' => $this->getAppId(), 'redirect_uri' => $redirectUri, 'response_type' => 'code'], $params));
    }
     ...
     *
     * @return bool Returns true if the authorization process is running
     */
     ...
     *
     * @return bool Returns true Meli Logout is in the query parameters
     */
     ...
     *
     * @return int Returns the ID of the MercadoLibre user, or 0
     ...
     * @param bool   $useAccessToken The path
     ...
     */
    protected function execute($method, $useAccessToken, $path, $params = [])
    {
        $getParams = [];
    ...
    protected function makeRequest($method, $url, $params, $ch = null)
    {
        ...
        if ($method == 'GET') {
            ...
            if ($params) {
                if (is_array($params)) {
                    ...
                    $opts[CURLOPT_HTTPHEADER] = array_merge($opts[CURLOPT_HTTPHEADER], ['Content-Type: application/json', 'Content-Length: '.strlen($params)]);
                }
            }
        }
        ...
        if ($content === false) {
            $e = new MeliApiException(['error_code' => curl_errno($ch), 'error' => ['message' => curl_error($ch), 'type' => 'CurlException']]);
            curl_close($ch);
            throw $e;
        }
        ...
        return ['statusCode' => $httpCode, 'body' => $body, 'headers' => $headers];
    }
     ...
     */
    protected function getUrl($subdomain, $path = '/', $params = [])
    {
     ...
     * @return string The URL for the given parameters
     */
    protected function getUrlForAPI($path, $params = [])
    {
